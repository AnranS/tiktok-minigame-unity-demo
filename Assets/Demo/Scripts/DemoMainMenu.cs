...
namespace TikTokMiniGameDemo
{
    /// <summary>
    /// Demo entry MonoBehaviour. Builds an accordion-style category list of TT.* API
    /// buttons at runtime, wires each one to a handler, and pipes results into the log
    /// panel. The UI itself is generated by <see cref="EditorTools.DemoSceneBuilder"/>.
    /// Demo 入口 MonoBehaviour。运行时构建可折叠的 TT.* 接口按钮列表，把每个按钮接到对应
    /// 调用，并把回调写到日志面板。UI 本身由 <see cref="EditorTools.DemoSceneBuilder"/> 生成。
    /// </summary>
    public class DemoMainMenu : MonoBehaviour
    {
        // -----------------------------------------------------------------
        // EN: Inspector references — wired by DemoSceneBuilder when it rebuilds the scene.
        // ZH: Inspector 引用——DemoSceneBuilder 重建场景时自动接线。
        // -----------------------------------------------------------------
        ...
        private void BuildFeatureButtons()
        {
            ...
            AddCategory("存档 / Storage",
                ...
                ("Save<DemoSaving>", DoSaveTyped),
                ("Load<DemoSaving>", DoLoadTyped),
                ("Delete<DemoSaving>", DoDeleteTyped),
        ...
        /// <summary>
        /// Demo POCO used to show <c>TT.Save&lt;T&gt;</c>. Any [Serializable] type works.
        /// 演示用 POCO，配合 <c>TT.Save&lt;T&gt;</c>。任何 [Serializable] 类型都可以。
        /// </summary>
        [Serializable]
        private class DemoSaving
        {
